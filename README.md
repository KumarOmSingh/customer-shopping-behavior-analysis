...
| File | Description |
|---|---|
| `data/customer_shopping_behavior.csv` | Raw dataset (3,900 rows, 18 columns) |
| `notebooks/Customer_Shopping_Behavior_Analysis.ipynb` | Python notebook — data cleaning, EDA, feature engineering, SQL DB load |
| `sql/customer_behavior_sql_queries.sql` | Business-question SQL queries (PostgreSQL) |
| `dashboard/customer_behavior_dashboard.pbix` | Power BI interactive dashboard |
| `assets/dashboard_screenshot.png` | Dashboard preview image (for quick viewing without Power BI) |
| `reports/Business_Problem_Document.pdf` | Business problem statement and deliverables |
| `reports/Customer_Shopping_Behavior_Analysis_Report.pdf` | Final project report |
| `reports/Customer_Shopping_Behavior_Analysis.pptx` | Stakeholder presentation deck |
...
1. **Clone the repo**
   ...
   ```
2. **Run the notebook** — `notebooks/Customer_Shopping_Behavior_Analysis.ipynb` (data import, cleaning, PostgreSQL connection)
3. **Run SQL queries** — Load cleaned data into PostgreSQL, then execute `sql/customer_behavior_sql_queries.sql`
4. **Open the dashboard** — `dashboard/customer_behavior_dashboard.pbix` in Power BI Desktop, connect to your SQL database
